...
`tree` 는 다음 넷을 이어 붙여 해시한 값입니다.

| 입력 | 왜 필요한가 |
| --- | --- |
| `git rev-parse HEAD` | 커밋이 바뀌면 다른 트리입니다 |
| `git status --porcelain` | 어떤 파일이 어떤 상태인지 |
| `git diff HEAD` | **내용**입니다. 경로와 상태 문자만 보면 이미 더티한 파일을 다시 고쳤을 때 `" M path"` 가 그대로여서 지문이 변하지 않습니다 |
| 미추적 파일의 경로와 `git hash-object` | `diff HEAD` 에 나오지 않는 새 파일의 내용 |

네 번째까지 넣어야 "검증한 뒤 한 줄이라도 고치면 값이 달라진다" 가 성립합니다. 앞의 둘만으로는 성립하지 않습니다. git 저장소가 아니거나 해시 도구가 없으면 `tree` 가 빈 문자열이 되고 그때는 신선도를 판정하지 않습니다.
